...
from util import rv_preaveraged, optimize_3param

if __name__ == "__main__":
    backend = torch
    device = torch.device("cpu")
    df = get_data()
    rv_arr_all = rv_preaveraged(df, backend=backend, device=device)
    est_rv = dict()
    lookback = 250
    ...
    for i in range(lookback,len(rv_arr_all)):
        ...
        if i%10 == 0:
            ...
            result.to_csv("result_garch_ito_unified_preaveraging.csv")
    result = pd.DataFrame(est_rv).T
    result.to_csv("result_garch_ito_unified_preaveraging.csv")